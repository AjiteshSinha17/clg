...
function cloudinarySignature(params, apiSecret) {
    ...
    return crypto_1.default.createHash('sha1').update(toSign + apiSecret).digest('hex');
}
...
async function createMarkerOnce(markerId, payload) {
...
async function sendOneSignalToPlayer(oneSignalId, title, body, data) {
...
async function sendPasswordResetEmailResend(email, resetLink) {
    if (!RESEND_API_KEY)
        return;
    const resend = new resend_1.Resend(RESEND_API_KEY);
    await resend.emails.send({
...
async function sendAccountActivityEmail(email, title, body) {
    if (!RESEND_API_KEY)
        return;
    const resend = new resend_1.Resend(RESEND_API_KEY);
    await resend.emails.send({
...
async function notifyPersonalChat(messageRefPath, chatId, msg, createdAtMs) {
    const senderId = msg.senderId ?? '';
    if (!senderId)
        return;
    const type = msg.type ?? 'text';
    const senderName = msg.senderName ?? 'New message';
    const body = type === 'image' ? 'Sent an image' : type === 'pdf' ? 'Sent a PDF' : (msg.content ?? '').toString();
    if (!body)
        return;
...
async function notifyCommunity(roomId, msg, createdAtMs, messageRefPath) {
    const senderId = msg.senderId ?? '';
    if (!senderId)
        return;
    const type = msg.type ?? 'text';
    ...
    const body = type === 'image' ? 'Shared an image' : type === 'pdf' ? 'Shared a PDF' : (msg.content ?? '').toString();
    if (!body)
        return;
    ...
    await Promise.all(usersSnap.docs.map(async (doc) => {
        ...
        const data = doc.data() ?? {};
        const prefs = data.notificationPreferences ?? {};
        if (prefs.communityChatEnabled === false)
            return;
...
async function notifyConnectionRequest(reqId, req, createdAtMs) {
...
function startFirestoreListeners() {
    ...
    db.collectionGroup('messages').onSnapshot(async (snap) => {
        ...
        for (const change of changes) {
            ...
            if (createTimeMs && createTimeMs < bootMs - 5000)
                continue;
            ...
            if (await markerExists(markerId))
                continue;
            const data = doc.data();
    ...
    db.collection('connection_requests')
        ...
        for (const change of changes) {
            ...
            await notifyConnectionRequest(doc.id, doc.data(), createTimeMs);
        }
    });
...
async function main() {
    ...
    app.post('/cloudinary/sign-upload', async (req, res) => {
        try {
            ...
            const { folder, publicId, timestamp, type } = req.body;
